...
async function searchDRKStore(query) {
    ...
    try {
        html = await Promise.race([
            fetchPageHtml(`${BASE_URL}/?s=${encodeURIComponent(query)}`),
            new Promise((_, reject) => setTimeout(() => reject(new Error("Stealth timeout")), 6000))
        ]);
    } catch {
        try {
    ...
    try {
        ...
        console.error(`❌ DRK Store parse error: ${e.message}`);
        return [];
    }
}
