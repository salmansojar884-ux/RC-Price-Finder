...
const BASE_URL = "https://rcmumbai.com";
...
async function searchRCMumbai(query) {
  console.log(`🔎 RC Mumbai search: ${query}`);
  ...
  try {
    ...
    $("li.product, .product.type-product").each((_, el) => {
      ...
      if (isBundleProduct(name, query)) {
        console.log(`🚫 RC Mumbai bundle removed: ${name}`);
        return;
      }
    ...
    console.log(`✅ RC Mumbai exact matches: ${results.length}`);
    return results;
  } catch (error) {
    console.error(`❌ RC Mumbai error: ${error.message}`);
    return [];
  }
}

module.exports = searchRCMumbai;