...
const BASE_URL = "https://hitechxyz.in";
...
async function searchHiTechXYZ(query) {
  console.log(`🔎 HiTech XYZ search: ${query}`);
  ...
  try {
    ...
    $("li.product, .product.type-product").each((_, el) => {
      ...
      if (isBundleProduct(name, query)) {
        console.log(`🚫 HiTech XYZ bundle removed: ${name}`);
        return;
      }
    ...
    console.log(`✅ HiTech XYZ exact matches: ${results.length}`);
    return results;
  } catch (error) {
    console.error(`❌ HiTech XYZ error: ${error.message}`);
    return [];
  }
}
