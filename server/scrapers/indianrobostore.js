...
const BASE_URL = "https://indianrobostore.com";
...
async function searchIndianRoboStore(query) {
  console.log(`🔎 Indian Robo Store search: ${query}`);
  ...
  try {
    ...
    $("li.product, .product.type-product, .product-grid-item").each((_, el) => {
      const container = $(el);

      let name = container
        .find(".wd-entities-title, .product-title, .woocommerce-loop-product__title, h2, h3")
        .first()
        .text()
        .trim();
      ...
      if (isBundleProduct(name, query)) {
        console.log(`🚫 Indian Robo Store bundle removed: ${name}`);
        return;
      }
    ...
    console.log(`✅ Indian Robo Store exact matches: ${results.length}`);
    return results;
  } catch (error) {
    console.error(`❌ Indian Robo Store error: ${error.message}`);
    return [];
  }
}
